...
void DeleteList(Int **head)
{
    Int *aux = (Int *) malloc(sizeof(Int)); // creating the nodes that we'll use to delete the list
    ...
    aux = *head; // deallocating the nodes
    while(aux->next != NULL)
    {
        aux1 = aux;
        aux = aux->next;
    ...
    free(aux); // deallocating the last node
    
    *head = NULL; // now it is a empty list
...
int main()
{
    ...
    DeleteList(&head); // calling the function
